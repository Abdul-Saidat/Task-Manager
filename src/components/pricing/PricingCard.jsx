...
function PricingCard({
  ...
  return (
    <div
      className={`rounded-3xl shadow-md p-4 lg:py-8 lg:px-5 flex flex-col min-h-107.5 gap-2.5 relative w-62.5 hover:shadow-xl transition duration-300 ${variantStyles[variant]}`}
    >
      <div className="flex flex-col h-full">
        ...
        <div>
          ...
          <p className="text-[30px] font-bold">
            {price}
            <span className="text-sm font-normal">{frequency}</span>
          </p>
        </div>
        <button
          className={`border rounded-lg px-2.5 py-2 mt-6 w-full ${buttonClassName}`}
        >
          Get started
        </button>
        <ul className="mt-3 lg:mt-6 space-y-3">
          {features.map((feature, index) => (
            <li className="flex items-center gap-2">
              <span>✓</span>
              <span key={index} className="text-sm leading-6">
                {feature}
              </span>
            </li>
          ))}
